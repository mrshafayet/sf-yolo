...
def check_dataset(data, autodownload=True):
    ...
    for k in 'train', 'val', 'names':
        assert k in data, emojis(f"dotav1.yaml '{k}:' field missing ❌")
    if isinstance(data['names'], (list, tuple)):  # old array format
        data['names'] = dict(enumerate(data['names']))  # convert to dict
    assert all(isinstance(k, int) for k in data['names'].keys()), 'dotav1.yaml names keys must be integers, i.e. 2: car'
...
def yaml_load(file='dotav1.yaml'):
    # Single-line safe yaml loading
    with open(file, errors='ignore') as f:
        return yaml.safe_load(f)


def yaml_save(file='dotav1.yaml', data={}):
